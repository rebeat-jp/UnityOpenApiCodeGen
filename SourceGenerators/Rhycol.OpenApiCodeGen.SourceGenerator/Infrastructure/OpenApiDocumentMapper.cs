...
{
    /// <summary>
    /// 正規化済みの OpenAPI node をドメインモデルに変換する。
    /// Maps a normalized OpenAPI node into domain models.
    /// </summary>
    internal sealed class OpenApiDocumentMapper
    {
        /// <summary>
        /// OpenAPI root node を変換する。
        /// Maps an OpenAPI root node.
        /// </summary>
        public static OpenApiDocument Parse(SpecNode root)
        {
            if (root is null)
            {
        ...
        /// </summary>
        private static OpenApiInfo ParseInfo(SpecNode? infoElement)
        {
        ...
        /// </summary>
        private static IReadOnlyDictionary<string, OpenApiPathItem> ParsePaths(SpecNode? pathsElement)
        {
        ...
        /// </summary>
        private static OpenApiOperation ParseOperation(SpecNode operationElement)
        {
        ...
        /// </summary>
        private static OpenApiParameter ParseParameter(SpecNode parameterElement)
        {
        ...
        /// </summary>
        private static OpenApiRequestBody? ParseRequestBody(SpecNode? requestBodyElement)
        {
            if (requestBodyElement is null || requestBodyElement.Value.ValueKind != SpecValueKind.Object)
            {
                return null;
            }
        ...
        /// </summary>
        private static IReadOnlyDictionary<string, OpenApiResponse> ParseResponses(SpecNode? responsesElement)
        {
        ...
        /// </summary>
        private static IReadOnlyDictionary<string, OpenApiMediaType> ParseContent(SpecNode? contentElement)
        {
            var content = new Dictionary<string, OpenApiMediaType>(StringComparer.Ordinal);
            if (contentElement is null || contentElement.Value.ValueKind != SpecValueKind.Object)
            {
                return content;
            }
            ...
            {
                if (contentProperty.Value.ValueKind != SpecValueKind.Object)
                {
                    continue;
                }
        ...
        /// </summary>
        private static OpenApiComponents ParseComponents(SpecNode? componentsElement)
        {
            if (componentsElement is null || componentsElement.Value.ValueKind != SpecValueKind.Object)
            {
        ...
        /// </summary>
        private static IReadOnlyDictionary<string, OpenApiSchema> ParseSchemas(SpecNode? schemasElement)
        {
            var schemas = new Dictionary<string, OpenApiSchema>(StringComparer.Ordinal);
            if (schemasElement is null || schemasElement.Value.ValueKind != SpecValueKind.Object)
            {
                return schemas;
            }
            ...
            {
                if (schemaProperty.Value.ValueKind != SpecValueKind.Object &&
                    schemaProperty.Value.ValueKind != SpecValueKind.True &&
                    schemaProperty.Value.ValueKind != SpecValueKind.False)
                {
                    continue;
                }
        ...
        /// Parses JSON Schema.
        /// </summary>
        private static OpenApiSchema ParseSchema(SpecNode? schemaElement)
        {
            ...
            if (schemaValue.ValueKind == SpecValueKind.True || schemaValue.ValueKind == SpecValueKind.False)
            {
                return new OpenApiSchema(booleanSchema: schemaValue.ValueKind == SpecValueKind.True);
            }

            if (schemaValue.ValueKind != SpecValueKind.Object)
            {
                return new OpenApiSchema();
            }
            ...
            if (itemsElement is not null && itemsElement.Value.ValueKind == SpecValueKind.Array && prefixItems.Count == 0)
            {
        ...
        /// </summary>
        private static OpenApiSchema? ParseSchemaOrNull(SpecNode? schemaElement)
        {
            if (schemaElement is null)
            {
                return null;
            ...
            if (value.ValueKind == SpecValueKind.Object ||
                value.ValueKind == SpecValueKind.True ||
        ...
        /// Parses a schema map.
        /// </summary>
        private static IReadOnlyDictionary<string, OpenApiSchema> ParseSchemaMap(SpecNode? element)
        {
            var schemas = new Dictionary<string, OpenApiSchema>(StringComparer.Ordinal);
            ...
            foreach (var property in element.Value.EnumerateObject())
            {
                if (property.Value.ValueKind == SpecValueKind.Object ||
                    property.Value.ValueKind == SpecValueKind.True ||
                    property.Value.ValueKind == SpecValueKind.False)
                {
        ...
        /// </summary>
        private static IReadOnlyList<OpenApiSchema> ParseSchemaList(SpecNode? element)
        {
            ...
            foreach (var item in element.Value.EnumerateArray())
            {
                if (item.ValueKind == SpecValueKind.Object ||
                    item.ValueKind == SpecValueKind.True ||
                    item.ValueKind == SpecValueKind.False)
                {
        ...
        /// </summary>
        private static void ParseSchemaOrBoolean(SpecNode? element, out OpenApiSchema? schema, out bool? allowed)
        {
            schema = null;
            allowed = null;
            if (element is null)
        ...
        /// </summary>
        private static IReadOnlyDictionary<string, IReadOnlyList<string>> ParseDependentRequired(SpecNode? element)
        {
        ...
        /// Parses $vocabulary.
        /// </summary>
        private static IReadOnlyDictionary<string, bool> ParseVocabulary(SpecNode? element)
        {
            ...
            foreach (var property in element.Value.EnumerateObject())
            {
                if (property.Value.ValueKind == SpecValueKind.True)
                {
                    values[property.Name] = true;
                }
                else if (property.Value.ValueKind == SpecValueKind.False)
                {
        ...
        {
            return !element.TryGetProperty(name, out var value) ? null : value.ValueKind == SpecValueKind.String ? value.GetString() : null;
        }
